...
Cohort support (measured 2026-07-17, 75 frames):
...
1.79); measured artifact incidence matches after the calibrated-chain
hot-pixel retune; limb and ring-edge rise widths agree at the few-tenths
level (pooled medians 2.66 vs 2.54 px and 2.46 vs 2.87 px); and the
sky-noise *level* matches at the median (2.2e-4 vs 2.1e-4 I/F) while the
distributional divergence remains floored by the quantization-scar gap
below.  FOM 3's small residual is not a PSF error -- FOM 2 pins the PSF
...
**Cassini WAC** (4 frames, limited).  The single WAC star frame pins the
tuned WAC kernel well (89 stars): cohort EE50/EE80 = 1.33/2.16 px against simulated 1.13/1.90 px, normalized W1 0.42 and 0.80 (from 1.32 and 3.16 before tuning).  The remaining WAC
statistics rest on 1-2 frames per stratum and are reported in the summary
JSON without distributional claims.
...
**Voyager ISS** (3 frames, limited).  The GEOMED star frame gives 9 real
star cutouts (EE50 median 1.22 px) against
three simulated ones (EE50 median 1.90 px) --
consistent with the retained interim sigma, but one frame is not a
distribution.  The vidicon read noise was tuned down from its high-gain
interim value (see the catalog); the sky-noise floor itself is
