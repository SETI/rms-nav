...
- a :data:`~spindoctor.feature.feature_type.NavFeatureType.RING_ANNULUS` carrying the rendered
  template, for :class:`~spindoctor.nav_technique.nav_technique_ring_annulus.RingAnnulusNav`;
- one :data:`~spindoctor.feature.feature_type.NavFeatureType.RING_EDGE` per rendered edge (inner
...
:class:`~spindoctor.nav_model.rings.ring_feature.RingFeature` data model carries the per-edge
...
:meth:`~spindoctor.nav_model.nav_model.NavModel.instances_for_obs` to build one instance per ring
...
2. Build :class:`~spindoctor.nav_model.rings.ring_feature.RingFeature` instances from the
...
1. Construct one
   ...
   :class:`~spindoctor.feature.feature.NavFeature` carrying the rendered template image, the
...
(``sd_create_simulated_image``). An operator specifies a Saturn-like ring stack —
...
mask. The downstream
:class:`~spindoctor.nav_technique.nav_technique_ring_annulus.RingAnnulusNav` correlates the
