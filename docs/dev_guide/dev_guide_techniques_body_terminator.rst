...
confidence formula's hard-zero gate forces confidence to zero.  When the final RMS residual
or the inlier count fails the spurious tests, the result is flagged
...
- ``spurious_min_inliers`` — int, default ``6`` (count).  Below this Tukey-inlier count the
  M-estimator covariance is no longer informative; the result is flagged spurious.
