...
``(dv, du)`` offset to the ``ObsSnapshot``'s FOV via
...
- **Snapshots only.**  Today the pipeline supports
  ...
  geometry) that the current bodies / rings helpers do not implement.
...
- The :class:`~nav.config.config.Config` loader's deep-merge rules apply
  ...
  override file replaces the per-source list wholesale (lists are
  replaced, not appended).  See
...
The :mod:`backplanes` package is not yet autodocumented under
:doc:`/api_reference`; the public surface is the four functions listed
above:
