...
technique shares.  Rotation fitting is currently disabled for ring annuli (the rotation pivot
...
currently override any ``feature_emission`` keys.  The search-window margin used by the
...
- :attr:`~nav.nav_technique.diagnostics.RingAnnulusDiagnostics.peak_to_runner_up_ratio` —
  ...
  quality to the next-best peak's outside the exclusion radius.  Carries no weight
  currently; the wiring is in place so a downstream recalibration can tune the alpha.
...
5. The covariance shape is always (2, 2); rotation fitting is currently disabled for ring
   annuli.  When
