...
SPICE_COVERAGE = json.loads((HERE / 'spice_coverage.json').read_text())['windows']
...
def load_saturn_ring_edges() -> dict[str, float]:
    ...
    cfg = yaml.safe_load(
        (REPO / 'src/spindoctor/config_files/config_310_saturn_rings.yaml').read_text()
    )
...
def edges_in_frame(rmin: float, rmax: float) -> list[str]:
    ...
    return [name for name, a in ring_edges().items() if rmin + margin <= a <= rmax - margin]
...
def star_vmags(ra_deg: float, dec_deg: float, fov_deg: float, vmag_max: float) -> list[float]:
    ...
    if _UCAC4 is None:
        ...
        _UCAC4 = UCAC4StarCatalog(
            os.environ.get('UCAC4_PATH', '/data/external-data/star-catalogs/UCAC4')
        )
    ...
    if ra - ra_half < 0 or ra + ra_half > 2 * math.pi:
        boxes = [
            (0.0, (ra + ra_half) % (2 * math.pi)),
            ((ra - ra_half) % (2 * math.pi), 2 * math.pi),
        ]
    else:
        boxes = [(ra - ra_half, ra + ra_half)]
    for ra0, ra1 in boxes:
        for star in _UCAC4.find_stars(
            ra_min=ra0, ra_max=ra1, dec_min=dec_min, dec_max=dec_max, vmag_max=vmag_max
        ):
...
def resolved(t: pm.SummaryTable, row: list[str]) -> bool:
    ...
    return (
        t.num(row, 'MINIMUM_PLANETOCENTRIC_LATITUDE') is not None
...
def phase_bin(phase: float | None) -> str:
    ...
    if phase is None:
        return 'unknown'
    for hi, name in ((30, '<30'), (60, '30-60'), (90, '60-90'), (120, '90-120')):
        if phase < hi:
            return name
    return '>120'
...
def scan_coiss() -> dict[str, list[dict]]:
    ...
    volsets = [
        dict(
            volset='COISS_2xxx',
            dataset='coiss_saturn',
            planet_kind='saturn_summary',
            ring_classes=True,
            main_ring_span=(70000.0, 145000.0),
        ),
        dict(
            volset='COISS_1xxx',
            dataset='coiss_cruise',
            planet_kind='jupiter_summary',
            ring_classes=False,
    ...
    _coiss_star_passes(
        out, star_frame_pool=star_frame_pool, body_star_pool=body_star_pool, tiny_pool=tiny_pool
    )
    return out


def _scan_coiss_volume(
    vs: dict,
    volume: str,
    *,
    ...
    ring_rows = {key(ring.get(r, 'FILE_SPECIFICATION_NAME')): r for r in ring.rows} if ring else {}
    sat_rows = {key(sat.get(r, 'FILE_SPECIFICATION_NAME')): r for r in sat.rows} if sat else {}
    ...
    for irow in idx.rows:
        ...
        rings_visible = rmin is not None and rmax is not None and rmin < span_hi and rmax > span_lo

        # ---- body_irregular
        for tgt, d, mr in res_moons:
            if tgt in IRREGULARS and 150 <= d <= 800:
                ph = moon.num(mr, 'CENTER_PHASE_ANGLE')
                out['body_irregular'].append(
                    cand(
                        'body_irregular',
                        volset,
                        volume,
                        ...
                        camera=camera,
                        dataset=dataset,
                        strata=(tgt, phase_bin(ph)),
                        selection={
                            'target': tgt,
                            'apparent_diameter_px': round(d, 1),
                            'center_phase_deg': ph,
                            'filter': filt,
                            'year': time,
                        },
        ...
        if redges and res_moons and not planet_disc and bobs is not None and abs(bobs) >= 1.0:
            tgt, d, mr = max(res_moons, key=lambda x: x[1])
            if d >= 50:
                ph = moon.num(mr, 'CENTER_PHASE_ANGLE')
                out['ring_plus_body'].append(
                    cand(
                        'ring_plus_body',
                        volset,
                        volume,
        ...
        if (
            ...
            and not res_moons
            and not planet_disc
            and bobs is not None
            and abs(bobs) >= 1.0
            and rres is not None
            and rres > 0
        ):
            best = None
            for name in redges:
            ...
            worst = min(
                (ring_edges()[n] * abs(math.sin(math.radians(bobs))) / rres, n) for n in redges
            )
            if worst[0] <= FRAME_PX * FRAME_PX / 16.0 and rres >= 0.3:
                out['ring_only_curved'].append(
                    cand(
                        'ring_only_curved',
                        volset,
                        volume,
        ...
        if len(res_moons) == 1 and not planet_disc:
            ...
            lat_range = lat_hi - lat_lo if lat_lo is not None and lat_hi is not None else None
            regular = tgt not in IRREGULARS
            sel = {
                'target': tgt,
                'apparent_diameter_px': round(d, 1),
                'center_phase_deg': ph,
                'lat_coverage_deg': (round(lat_range, 1) if lat_range is not None else None),
                'filter': filt,
                'year': time,
            }
            if (
                regular
                and 700 <= d <= 950
                and not rings_visible
                and ph is not None
                and ph < 110
                and lat_range is not None
                and lat_range > 160
            ):
                out['body_full_fov'].append(
                    cand(
                        'body_full_fov',
                        volset,
                        volume,
            ...
            if 1080 <= d <= 1500 and not rings_visible:
                out['body_partial_overflow'].append(
                    cand(
            ...
            if (
                ...
                and ph < 90
                and lat_range is not None
                and 25 <= lat_range < 120
            ):
                out['body_mostly_offscreen'].append(
                    cand(
            ...
            if regular and ph is not None and ph >= 95 and 100 <= d <= 700 and not rings_visible:
                out['high_phase_terminator'].append(
                    cand(
        ...
        if (
            ...
            out['below_resolution_body'].append(
                cand(
...
def _coiss_star_passes(
    ...
    *,
    ...
    for fr in sample(star_frame_pool, len(star_frame_pool)):
        ...
        vm = star_vmags(fr['ra'], fr['dec'], FOV_DEG[('COISS', fr['camera'])], lim + 2.0)
        n_bright = sum(1 for v in vm if v <= lim)
        sel = {
            'texp_s': fr['texp'],
            'maglim': round(lim, 2),
            'star_vmags': [round(v, 2) for v in vm[:5]],
            'n_detectable': n_bright,
            'filter': fr['filt'],
            'year': fr['year'],
        }
        if n_bright == 2 and (len(vm) < 3 or vm[2] >= vm[1] + 1.5):
            out['two_bright_stars_no_body'].append(
                cand(
                    'two_bright_stars_no_body',
                    fr['volset'],
                    fr['volume'],
                    filespec=fr['filespec'],
                    mission='COISS',
                    camera=fr['camera'],
                    dataset=fr['dataset'],
                    strata=(fr['year'],),
            ...
            out['one_bright_star_no_body'].append(
                cand(
            ...
            out['star_dominated'].append(
                cand(
                    ...
                    camera=fr['camera'],
                    dataset=fr['dataset'],
                    strata=(fr['year'], min(n_bright, 8)),
                    selection=sel,
                )
            )
        elif n_bright == 0 and fr['texp'] <= 3.0 and len(vm) == 0:
            out['negative_cases'].append(
                cand(
                    'negative_cases',
                    fr['volset'],
                    fr['volume'],
    ...
    for fr in sample(body_star_pool, 400):
        lim = maglim('COISS', fr['camera'], fr['texp'])
        vm = star_vmags(fr['ra'], fr['dec'], FOV_DEG[('COISS', fr['camera'])], lim)
        if len(vm) >= 3:
            out['stars_plus_body'].append(
                cand(
                    'stars_plus_body',
                    fr['volset'],
                    fr['volume'],
        ...
        if not vm:
            out['negative_cases'].append(
                cand(
                    ...
                    selection={
                        'type': 'tiny_body_no_stars',
                        'bodies_px': fr['bodies'],
                        'texp_s': fr['texp'],
                        'maglim': round(lim, 2),
                        'n_detectable_stars': 0,
                        'filter': fr['filt'],
                        'year': fr['year'],
                    },
                )
...
def scan_nh() -> dict[str, list[dict]]:
    ...
    for volume in pm.volumes(volset):
        ...
        for kind in (
            ...
            if t:
                body_specs |= {
                    (t.get(r, 'FILE_SPECIFICATION_NAME') or '').rsplit('.', 1)[0].strip().lower()
                    for r in t.rows
                }
        for row in sup.rows:
            filespec = (sup.get(row, 'FILE_SPECIFICATION_NAME') or '').strip()
            if not filespec:
                continue
            ...
            if ckey not in cache:
                cache[ckey] = star_vmags(ra, dec, FOV_DEG[('NHLORRI', 'LORRI')], lim + 2.0)
            vm = cache[ckey]
            n_bright = sum(1 for v in vm if v <= lim)
            sel = {
                'texp_s': texp,
            ...
            if n_bright == 2 and (len(vm) < 3 or vm[2] >= vm[1] + 1.5):
                ...
                out['star_dominated'].append(
                    cand(
                        ...
                        camera='LORRI',
                        dataset=dataset,
                        strata=('NH', time_full[:4], min(n_bright, 8)),
                        selection=sel,
                    )
                )
    return out
...
def scan_go() -> dict[str, list[dict]]:
    ...
    for volume in pm.volumes(volset):
        ...
        ring_specs = {ring.get(r, 'FILE_SPECIFICATION_NAME') for r in ring.rows} if ring else set()
        sky_rows = {sky.get(r, 'FILE_SPECIFICATION_NAME'): r for r in sky.rows}
        ...
        for irow in idx.rows:
            ...
            if (
                no_body
                and filespec not in ring_specs
        ...
        if n_bright == 0 and n_faint >= 3:
            out['faint_stars'].append(
                cand(
                    ...
                    selection={
                        ...
                        'maglim': round(lim, 2),
                        'n_bright': 0,
                        'n_faint_within_2mag': n_faint,
                        'faintest_usable_vmags': [round(v, 2) for v in vm[:6]],
                        'target': fr['tgt'],
                        'filter': fr['filt'],
                        'year': fr['year'],
                    },
            ...
            out['negative_cases'].append(
                cand(
                    ...
                    camera='SSI',
                    dataset=dataset,
                    strata=('GOSSI', 'empty_sky'),
                    selection={
                        'type': 'empty_sky',
                        'texp_s': fr['texp'],
                        'maglim': round(lim, 2),
                        'n_stars_within_2mag': 0,
                        'target': fr['tgt'],
                        'filter': fr['filt'],
                        'year': fr['year'],
                    },
    ...
    for volset in ('VGISS_6xxx',):
        for volume in pm.volumes(volset):
            ...
            if moon:
                for row in moon.rows:
                    if resolved(moon, row):
                        moon_specs_resolved.add(moon.get(row, 'FILE_SPECIFICATION_NAME'))
            sat_specs_resolved = set()
            if sat:
                for row in sat.rows:
                    if resolved(sat, row):
                        sat_specs_resolved.add(sat.get(row, 'FILE_SPECIFICATION_NAME'))
            ring_specs = (
                {ring.get(r, 'FILE_SPECIFICATION_NAME') for r in ring.rows} if ring else set()
            )
            ...
            for irow in idx.rows:
                ...
                nothing_resolved = (
                    rawspec not in moon_specs_resolved
                ...
                if (
                    ...
                    and texp_s >= 1.0
                    and (
                        rawspec in ring_specs
                        or rawspec in moon_specs_resolved
                        or rawspec in sat_specs_resolved
                    )
                ):
                    stray_pool.append(rec)
                if nothing_resolved and texp_s <= 0.5 and tgt in ('DARK', 'SKY', 'STAR', 'SATURN'):
                    neg_pool.append(rec)
    ...
    for fr in neg_pool[:10]:
        out['negative_cases'].append(
            cand(
                ...
                camera=fr['camera'],
                dataset='vgiss',
                strata=('VGISS', 'empty_short_exp'),
                selection={
                    'type': 'empty_short_exposure',
                    'target': fr['tgt'],
                    'texp_s': fr['texp'],
                    'filter': fr['filt'],
                    'year': fr['year'],
...
def main() -> None:
    ...
    ap.add_argument(
        '--batch', type=int, default=2, help='batch number (selects quotas and output name)'
    )
    args = ap.parse_args()
    if args.batch not in QUOTAS_BY_BATCH:
        ap.error(f'unsupported --batch {args.batch}; supported batches: {sorted(QUOTAS_BY_BATCH)}')
    ...
    if args.batch >= 2 and results.get('scattered_light'):
        ...
        results['scattered_light'] = prescan(
            results['scattered_light'], keep=quotas.get('scattered_light', 12) * 2
        )
