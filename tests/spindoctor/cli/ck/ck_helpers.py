"""Hermetic SPICE helpers for the C-kernel writer tests.
...
This is a plain module rather than a ``conftest``: it holds the kernel texts,
the analytic attitude histories and the file writers the tests share, while the
fixtures that manage the process-global SPICE pool live in the package's
``conftest.py``.
"""
