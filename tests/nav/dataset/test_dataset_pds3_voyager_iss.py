...
def test_voyager_iss_yield_basic(ds_voyager_iss) -> None:
    ret = ds_voyager_iss.yield_filenames_index(max_filenames=2, retrieve_files=False)
...
def test_voyager_iss_yield_vol_end(ds_voyager_iss) -> None:
    ret = ds_voyager_iss.yield_filenames_index(vol_end='VGISS_5102', retrieve_files=False)
...
def test_voyager_iss_yield_img_end_num(ds_voyager_iss) -> None:
    ret = ds_voyager_iss.yield_filenames_index(img_end_num=1469550, retrieve_files=False)
    ret2 = [x[0].as_posix() for x in ret]
    assert len(ret2) == 117
