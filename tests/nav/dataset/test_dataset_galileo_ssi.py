...
def test_galileo_ssi_yield_vol_start() -> None:
    ret = _DS.yield_filenames_index(max_filenames=1, vol_start='GO_0020',
...
def test_galileo_ssi_yield_vol_end() -> None:
    ret = _DS.yield_filenames_index(vol_end='GO_0003', retrieve_files=False)
...
def test_galileo_ssi_yield_img_start_num() -> None:
    ret = _DS.yield_filenames_index(max_filenames=2, img_start_num=59468500,
                                          retrieve_files=False)
    ret2 = [x[0].as_posix() for x in ret]
    assert len(ret2) == 2
...
def test_galileo_ssi_yield_img_end_num() -> None:
    ret = _DS.yield_filenames_index(img_end_num=59468500, retrieve_files=False)
    ret2 = [x[0].as_posix() for x in ret]
    assert len(ret2) == 181
...
def test_galileo_ssi_yield_volumes() -> None:
    ret = _DS.yield_filenames_index(volumes=['GO_0003', 'GO_0020'],
                                          retrieve_files=False)
    ret2 = [x[0].as_posix() for x in ret]
