...
def test_cassini_iss_camera(ds_cassini_iss) -> None:
    ...
    ret = list(ds_cassini_iss.yield_image_files_index(max_filenames=1, volumes=['COISS_1001'],
                                                      arguments=arguments))
    assert len(ret) == 1
...
def test_cassini_iss_group_botsim(ds_cassini_iss) -> None:
    ret = list(ds_cassini_iss.yield_image_files_index(group='botsim', img_start_num=1294562768,
                                                      img_end_num=1294562949))
    assert len(ret) == 3
