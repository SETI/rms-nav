...
Schema version bumps to 2. New/changed keys (all optional, so existing scenes
...
- rings: `ring_system` block (list of features with `kind`, `tau`, orbit
  ...
  shared `geometry` with opening angle / lit-side flag). The legacy
  `rings:` list stays valid (internally converted to a two-edge ringlet
  system) so the existing catalog and baselines do not churn in the same PR.
...
Each phase is independently landable and keeps the full suite green.

- **A. Skeleton + independence.** `sim/forward/` package with stage
  interfaces; `render_combined_model` re-plumbed; import-graph test;
  current behavior reproduced (golden-image tolerance) so baselines survive.
...
- **I. Sweeps + report.** Section 8 axes wired into sim_sweep, report
  rewrite, terminator emission + `BodyTerminatorNav` recalibration (#223).
...
### 15.1 The identity invariant (governs every phase)

**A stage whose scene block is absent is the identity function.** No new
stage, block, or default may change the rendered output of any existing
scene file. Concretely:
...
- Per-instrument artifact defaults are *not* implied by naming an
  ...
  (then per-mode overrides apply on top). Existing instrument-named scenes
  therefore render bit-identically.
- Phase A must land with a test that renders every scene in the current
  catalog before/after the re-plumbing and asserts allclose with
  rtol=1e-12 (exact reproduction is expected; the tolerance exists for BLAS
  reassociation only). Baselines and tier files are not regenerated in any
  phase unless a scene file itself is changed, and then only that scene's.
- The legacy `rings:` list remains valid indefinitely (converted internally
  to a two-edge ringlet system). This is a deliberate, operator-approved
  exception to the no-compatibility-shims rule, scoped to scene files.
...
- Stages mutate `frame` in place, in the fixed order of Section 3.3; each
  ...
  `derive_effect_seed(random_seed, '<stage-name>')`. Stage names are part
  of the compatibility contract (renaming one changes its stream).
- **Unit chain.** Scene signal units are normalized I/F-like values in
  ...
  4095 DN x 21 e-/DN, antiblooming CCD], vgiss n/a — see below). Poisson is applied in electrons, read noise in
  electrons, then `DN = electrons / gain_e_per_dn + bias_dn`, quantized,
...
- **Star flux.** `total_electrons = star_flux_e_per_s_vmag0 *
  ...
  e-/s. The old peak-based `2.512^-(vmag-4)` path remains for scenes
  without an `optics` block (identity invariant).
...
`delta(theta)`: wrap-around 1-D Gaussian random field on [0, 2*pi),
generated as Fourier series with coefficients ~ N(0, S(k)),
`S(k) proportional to exp(-(k * corr_rad / 2)^2)`, scaled so
std(delta) = `limb_relief_rms`; seeded from the body's crater seed
derivation. Applied as `ellipse_dist_adjusted = ellipse_dist / (1 +
...
`CURRENT_SCHEMA_VERSION` becomes 2; the loader continues to accept
version 1 files unchanged (they contain no v2 keys, so the identity
invariant makes the version label the only difference). `save_sim_scene`
always writes 2. New keys, all optional (types; defaults in parentheses):
...
The GUI is **out of scope**: new blocks are YAML-only; the GUI must load
and re-save scenes containing them without loss (round-trip test) but gets
no new controls.
...
- **A**: catalog renders bit-identical; import-graph test green; stage
  registry + SimFrame landed.
- **B**: PSF/smear/SPK/detector stages meet 15.2-15.3; identity invariant
  test extended to every new block; performance budget met.
...
- **I**: sweep tables regenerate; report restructured per Section 9;
  `TERMINATOR_ARC` emitted and `BodyTerminatorNav` recalibrated (#223).
...
- **Standard-class OK: phases B, C, D, G, I.** Well-specified
  ...
  per-phase acceptance criteria are pinned, and the identity-invariant and
...
- **Frontier-class required: phases A, F, H.**
  - **A** (skeleton + de-circularization): an architecture task whose
    ...
    phase; wrong early decisions are expensive and invisible.
