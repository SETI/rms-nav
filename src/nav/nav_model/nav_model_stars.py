...
class NavModelStars(NavModel):
        ...
        **kwargs: Any,
        ...
        for star in star_list1:
            ...
            if star.johnson_mag_v is None or star.johnson_mag_b is None:
                star.johnson_mag_v = star.vmag
                star.johnson_mag_b = (
                    star.vmag + SCLASS_TO_B_MINUS_V[clean_sclass(star.spectral_class)]
                )
        ...
        if self._stars_config.proper_motion:
            ...
            ra_dec_list = [(cast(float, x.ra), cast(float, x.dec)) for x in star_list2]
        ...
        if radec_movement is not None:
            uv1 = obs.uv_from_ra_and_dec(
                ra_list - radec_movement[0], dec_list - radec_movement[1], apparent=True
            )
            uv2 = obs.uv_from_ra_and_dec(
                ra_list + radec_movement[0], dec_list + radec_movement[1], apparent=True
            )
        else:
            uv1 = obs.uv_from_ra_and_dec(ra_list, dec_list, tfrac=0.0, apparent=True)
            uv2 = obs.uv_from_ra_and_dec(ra_list, dec_list, tfrac=1.0, apparent=True)
        ...
        for star, (ra_pm, dec_pm), u, v, u1, v1, u2, v2 in zip(
            star_list2,
            ra_dec_pm_list,
            u_list,
            v_list,
            ...
            if (
                u <= obs.extfov_u_min + psf_size_half_u
                or u >= obs.extfov_u_max - psf_size_half_u
                or v <= obs.extfov_v_min + psf_size_half_v
                or v >= obs.extfov_v_max - psf_size_half_v
                or u1 <= obs.extfov_u_min + psf_size_half_u
                or u1 >= obs.extfov_u_max - psf_size_half_u
                or v1 <= obs.extfov_v_min + psf_size_half_v
                or v1 >= obs.extfov_v_max - psf_size_half_v
                or u2 <= obs.extfov_u_min + psf_size_half_u
                or u2 >= obs.extfov_u_max - psf_size_half_u
                or v2 <= obs.extfov_v_min + psf_size_half_v
                or v2 >= obs.extfov_v_max - psf_size_half_v
            ):
                self.logger.debug(
                    f'Star {star.pretty_name:9s} U {u:9.3f} V {v:9.3f} is off the edge'
                )
                continue
        ...
        self.logger.debug(f'Found {len(star_list3)} new stars')
    ...
    def stars_list_for_obs(
        ...
        if radec_movement is None:
            self.logger.debug(
                f'Retrieving star list with VMAG range {mag_vmin:.2f} to {mag_vmax:.2f}'
            )
        else:
            self.logger.debug(
                f'Retrieving star list with RA/DEC movement {radec_movement[0]:.5f}, '
        ...
        for catalog_name in self._stars_config.catalogs:
            ...
            for mag_min, mag_max in zip(magnitude_list[:-1], magnitude_list[1:]):
                ...
                star_list = self._stars_list_for_obs(
                    catalog_name.lower(),
                    ra_min,
                    ra_max,
                    dec_min,
            ...
            for i in range(len(full_star_list)):
                for j in range(i + 1, len(full_star_list)):
                    ...
                    if (
                        ...
                        if (
                            cast(float, full_star_list[j].vmag)
    ...
    def _create_model(
        ...
        if ra_dec_predicted is not None:
            ...
            radec_movement = (
                ra_dec_predicted[6] * self._obs.texp / 2,
        ...
        for star in star_list:
            ...
            if (
                ...
                or u_int >= model.shape[1] - psf_size_half_u
                or v_int < psf_size_half_v
                or v_int >= model.shape[0] - psf_size_half_v
            ):
                continue
        ...
        if _DEBUG_STARS_MODEL_IMGDISP:
            ...
            cast(Any, ImageDisp)(
                [model], canvas_size=(1024, 1024), enlarge_limit=10, auto_update=True
            )
            tk.mainloop()
        ...
        if create_annotations:
            ...
            for star in star_list:
                if (
                    star.conflicts
                    and star.conflicts != 'STAR'
                    and not star.conflicts.startswith('REFINEMENT')
                ):
        ...
        self.logger.debug(f'  Star model min: {np.min(model)}, max: {np.max(model)}')
    ...
    def _mark_conflicts_obj(
        ...
        meshgrid = Meshgrid.for_fov(
            obs.fov,
            origin=(star.u - star_slop, star.v - star_slop),
            limit=(star.u + star_slop, star.v + star_slop),
        )
