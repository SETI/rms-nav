...
every scene block: the per-object stars entries, the ring_system block, the
optics sub-blocks (PSF, smear, distortion, ghosts, stray light), the noise,
detector, artifacts, and spk_error blocks, plus the primitive scalar checks
they all share.  Each block's key inventory lives beside its checker (unknown keys fail
