#!/usr/bin/env python3
"""sd_mosaic_display -- interactive viewer for ring and body reprojection/mosaic files.

Entry points
------------
sd_mosaic_display         -- dispatches on the first positional argument (``rings`` or ``body``)
sd_mosaic_display_rings   -- equivalent to ``sd_mosaic_display rings ...``
sd_mosaic_display_body    -- equivalent to ``sd_mosaic_display body ...``

Usage
-----
sd_mosaic_display_rings [options] FILE [FILE ...]
sd_mosaic_display_body  [options] FILE [FILE ...]
...
#   python src/spindoctor/cli/sd_mosaic_display.py rings file1.fits
...
from spindoctor.cli.reproj.args import add_display_args
from spindoctor.ui.mosaic_viewer.body_window import BodyMosaicWindow
from spindoctor.ui.mosaic_viewer.projections import ProjectionKind
from spindoctor.ui.mosaic_viewer.ring_window import RingMosaicWindow
...
def _build_parser(mode: str) -> argparse.ArgumentParser:
    """Build the :class:`argparse.ArgumentParser` for ``sd_mosaic_display``.
    ...
    Returns:
        ...
        paths), display/stretch options from :func:`spindoctor.cli.reproj.args.add_display_args`,
        and for ``mode='body'`` only, ``--projection`` (initial map projection).
    """
    description = {
        'rings': (
            'Display ring reprojection or mosaic files produced by sd_mosaic_rings. '
            'Supports tiled dynamic zoom, stretch, color-by, show-radii, EW plot, '
            'radial-slice, axis ticks, and Save-FOV.'
        ),
        'body': (
            'Display body reprojection or mosaic files produced by sd_mosaic_body. '
    ...
    parser = argparse.ArgumentParser(
        prog=f'sd_mosaic_display_{mode}',
...
def _run_rings(args: argparse.Namespace) -> None:
    """Open a :class:`~spindoctor.ui.mosaic_viewer.ring_window.RingMosaicWindow` and run Qt.
...
def _run_body(args: argparse.Namespace) -> None:
    """Open a :class:`~spindoctor.ui.mosaic_viewer.body_window.BodyMosaicWindow` and run Qt.
    ...
    if ('-h' in args_list or '--help' in args_list) and (
        ...
        top_parser = argparse.ArgumentParser(
            ...
            description=(
                ...
                'passed to that mode (see sd_mosaic_display_rings / '
                'sd_mosaic_display_body).'
            ),
            formatter_class=argparse.ArgumentDefaultsHelpFormatter,
            epilog=(
                'Usage:\n'
                '  sd_mosaic_display rings [options] FILE [FILE ...]\n'
                '  sd_mosaic_display body  [options] FILE [FILE ...]'
            ),
        )
        top_parser.print_help()
        sys.exit(0)
    if not args_list or args_list[0] not in ('rings', 'body'):
        argparse.ArgumentParser(prog='sd_mosaic_display').error(
            'Usage: sd_mosaic_display <rings|body> [options] FILE [FILE ...]'
        )
    mode = args_list[0]
...
def rings_main() -> None:
    """Entry point for ``sd_mosaic_display_rings``; prepends ``rings`` to argv."""
    sys.argv = [sys.argv[0], 'rings', *sys.argv[1:]]
    main()


def body_main() -> None:
    """Entry point for ``sd_mosaic_display_body``; prepends ``body`` to argv."""
